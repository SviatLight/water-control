...
const WaterControl = () => {
  ...
  useEffect(() => {
    if (Object.keys(dbUser).length > 0) {
      ...
      drinkTodayPercent = Math.round(totalDrink / (dbUser.userWeight * 30) * 100);
      if(totalDrink !== 0) setWaterHistory(dbUser.historyOfDrunkWater[curDate])
      drawWaterPercent()
    }
  }, [dbUser]);
  ...
  return (
    <div className={style.wrapper}>
      <div className={style.wrapper_target}>
        <div id="watter_drunk" className={style.watter_drunk}>{drunkToday.toString()}</div>
