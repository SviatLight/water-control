...
import {addDays, addWeeks, addYears, endOfWeek, startOfMonth, startOfWeek, startOfYear} from "date-fns";
...
const AnalyticsParams = ({startDate, endDate, handleChange}) => {
  ...
  const staticRanges = [
    ...
    genStaticRange('Yesterday', addDays(new Date(), -1), addDays(new Date(), -1)),
    genStaticRange('This week', getWeekStart(new Date()), new Date()),
    genStaticRange('Week', addDays(new Date(), -7), new Date()),
    genStaticRange('Previous week', getWeekStart(addWeeks(new Date(), -1)),
