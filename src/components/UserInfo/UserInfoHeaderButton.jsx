...
const UserInfoHeaderButton = ({thisPageIndex, currentPageIndex, clickHandler, currentUser, userKey, children}) => {
  let className
  let disabled = false
  ...
  return (
    <button className={className} disabled={disabled} tabIndex={thisPageIndex} onClick={clickHandler}>
      {children}
    </button>
  );
};
